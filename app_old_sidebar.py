...
if 'authenticated' in st.session_state and st.session_state.authenticated:
    ...
    # Not authenticated - hide sidebar
    st.set_page_config(
...
<style>
    ...
    .login-page [data-testid="collapsedControl"] {
        display: none !important;
    }
...
if st.session_state.authenticated:
    # Add class for authenticated state
    st.markdown('<div class="authenticated-page">', unsafe_allow_html=True)
    # Top nav
    ...
    with col_nav2:
        st.markdown(f"""
        <div style="text-align: center; background: white; padding: 0.5rem; border-radius: 10px;">
            <div style="font-size: 2rem;">👤</div>
            <div style="font-size: 0.8rem; color: #667eea; font-weight: 600;">{st.session_state.user_name}</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with st.sidebar:
        ...
        # Main navigation would go here (modules, etc.)
        st.markdown("### 📋 Navigation")
        st.info("Module navigation coming soon")
        
        # Push account menu to bottom with spacer
        ...
        <div style="text-align: center; padding: 1rem; background: rgba(255,255,255,0.1); border-radius: 10px; margin-bottom: 1rem;">
            <div style="width: 60px; height: 60px; border-radius: 50%; background: white; 
                        ...
                        font-size: 1.5rem; color: #667eea; font-weight: 600;">
                {st.session_state.user_name[0].upper()}
            </div>
            <div style="color: white; font-weight: 600; margin-top: 0.5rem;">{st.session_state.user_name}</div>
            <div style="color: rgba(255,255,255,0.8); font-size: 0.85rem;">{st.session_state.user_role.title()}</div>
        </div>
        """, unsafe_allow_html=True)
        
        # Account menu as compact list
        st.markdown('<div style="background: rgba(255,255,255,0.1); border-radius: 10px; padding: 0.5rem;">', unsafe_allow_html=True)
        ...
        for icon, label, view in menu_items:
            ...
            bg_color = "rgba(255,255,255,0.2)" if is_active else "transparent"
...
# LOGIN VIEW - Add class for login page
